...
public class InChIToStructure {
	...
	/**
	 * Given a pre-constructed InchiInput structure, construct a CDK IAtomContainer.
	 * 
	 * This constructor allows for the possibility of another toolkit (such as
	 * OpenChemLib) or an embedding web page to pre-contruct an InchiInput itself by
	 * running JNA-InChI code, perhaps from a MOL file, then passing that to the
	 * CDK. This allows InchiInput to be a common connctor between projects,
	 * allowing cross-programming and allows for a wider variety of structure sources.
	 *
	 * @param input   InchiInput from some other toolkit
	 * @param builder A builder to use, possibly
	 *                DefaultChemObjectBuilder.getInstance()
	 * @throws CDKException
	 * @author Bob Hanson
	 */
    protected InChIToStructure(InchiInput input, IChemObjectBuilder builder) throws CDKException {
    	generateAtomContainerFromInchiInput(builder, input);
	}
    ...
    protected void generateAtomContainerFromInchi(IChemObjectBuilder builder) throws CDKException {    	
        generateAtomContainerFromInchiInput(builder, output.getInchiInput());
    }

    /**
     * Allows for InchiInput that comes from a source other than an InchiInputFromInchiOutput instance.
     * For example, from reading an auxinfo or MOL file.
     * 
     * @param builder
     * @param input
     * @throws CDKException
     */
	private void generateAtomContainerFromInchiInput(IChemObjectBuilder builder, InchiInput input) throws CDKException {
		molecule = builder.newInstance(IAtomContainer.class);
		Map<InchiAtom, IAtom> inchiCdkAtomMap = new HashMap<>();

		List<InchiAtom> atoms = input.getAtoms();
		int nDim = getDimension(atoms);
		for (int i = 0; i < atoms.size(); i++) {
			...
			// Coordinates can come from auxinfo or MOL file input.
			// We pass these on.
			switch (nDim) {
			case 0:
				break;
			case 2:
				...
				break;
			case 3:
				cAt.setPoint3d(new Point3d(iAt.getX(), iAt.getY(), iAt.getZ()));
				break;
			}
			...
			} else if (radical == InchiRadical.SINGLET || radical == InchiRadical.TRIPLET) {
		...
		for (InchiStereo stereo0d : stereos) {
			if (stereo0d.getType() == InchiStereoType.Tetrahedral || stereo0d.getType() == InchiStereoType.Allene) {
				...
				IAtom[] neighbors = new IAtom[] { inchiCdkAtomMap.get(neighbours[0]),
						inchiCdkAtomMap.get(neighbours[1]), inchiCdkAtomMap.get(neighbours[2]),
    ...
    /**
     * Determine the dimensionality of the molecule
     * @param atoms
     * @return 0, 2, or 3
     */
	private int getDimension(List<InchiAtom> atoms) {
		int dim = 0;
		for (int i = atoms.size(); --i >= 0;) {
			InchiAtom a = atoms.get(i);
			if (a.getZ() != 0) {
				return 3;
			}
			if (dim == 0 && (a.getX() != 0 || a.getY() != 0)) {
				dim = 2;
			}
		}
		return dim;
